...
## 너의 도구
```bash
pm-copilot doctor                          # 지금 상태 확인
pm-copilot set_config me.name="홍길동"      # 값 저장(여러 개 한 번에)
pm-copilot schedule_brief                  # 예약 등록 문구 보기
```
...
```bash
pm-copilot set_config language=auto   # en·ja·zh 등으로 고정 가능
```
## 진행 순서
...
### 5단계. 매일 언제 받을지 (예약)
```bash
pm-copilot set_config brief.schedule="0 9 * * 1-5"   # 평일 오전 9시
pm-copilot schedule_brief                            # 등록 문구 출력
```
