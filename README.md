...
기획자(PM)를 위한 **사수형 업무 도구(스킬 42종)** — 데일리 브리핑·사수 검토부터 기획서·화면설계·IR·프로젝트 관리·QA·VOC까지. 매일 아침 놓치기 쉬운 것을 사수처럼 정리해 슬랙/노션으로 보내고, **일하는 내내 선제적으로 챙긴다**(작업물 저장할 때마다 검토, 하루 1~2회 이상신호 체크인). Slack·Notion·Jira·Figma·Microsoft 365·Google 등 **커넥터 11종** 연동.
...
- **세션 열면**: 오늘 브리핑·할 일·미결·묵힌 백로그를 먼저 짚고, 예약 안 걸었으면 "매일 예약 걸까요?" (처음 깔았으면 "설정 도와드릴까요?")
- **요청할 때마다**: 상황 감지 → 맞는 사수 관점 선제 주입(기획엔 '왜'·비목표, 배포엔 QA·롤백 등). 세션당 1회, 소음 방지.
...
- **매일 자동 루틴**: 예약 걸면 아침마다 슬랙으로 브리핑(무인 전송은 예약 실행에서만).
...
├─ commands/                    슬래시 명령 49개 (/pm-copilot:...)
...
## 참고 / 권장
- **모델·플랜:** 이 플러그인을 제대로 쓰려면 **Claude Max 구독**을 권장한다 — 매일 브리핑·상시 훅·심층 리서치가 사용량에 막히지 않는다. 복잡한 검토·리서치·다각도 분석엔 **Opus(최신) + ultracode**, 빠른 대안으로 **Fable** 을 쓰면 품질이 확 달라진다.
